...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/covidbed'
db=SQLAlchemy(app)
...
def admin():
    ...
    if request.method=="POST":
        ...
        password=request.form.get('password')
        if(username=="abdul" and password=="abdul"):
...
def slotbooking():
    ...
    if request.method == "POST":
        ...
        # Decrement bed count based on the bed type
        ...
        if dbb:
            if bedtype == "NormalBed":
                ...
                if seat > 0:
                    ...
                    flash("No Normal Beds available", "warning")
                    return render_template("booking.html", query=query, query1=query1)
                ...
                if seat > 0:
                    ...
                    flash("No HICU Beds available", "warning")
                    return render_template("booking.html", query=query, query1=query1)
            elif bedtype == "ICUBed":
                seat = dbb.icubed
                if seat > 0:
                    ...
                    flash("No ICU Beds available", "warning")
                    return render_template("booking.html", query=query, query1=query1)
                ...
                if seat > 0:
                    ...
                    flash("No Ventilator Beds available", "warning")
                    return render_template("booking.html", query=query, query1=query1)
            else:
                flash("Invalid bed type", "warning")
                return render_template("booking.html", query=query, query1=query1)
